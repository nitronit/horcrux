...
type TestValidator struct {
	...
	PrivKeyShares []thresholdsigner.CosignerKey
	Threshold     int
}
...
func (tv *TestValidator) generateShares(filePVKey privval.FilePVKey) error {
	...
	shares, err := thresholdsigner.CreateCosignerShares(filePVKey, int64(tv.Threshold), int64(len(tv.Signers)))
	if err != nil {
		return err
	}
	...
	for i, s := range tv.Signers {
		...
		if err := thresholdsigner.WriteCosignerShareFile(shares[i], privateFilename); err != nil {
			return err
		}
	}
	return nil
